...
class Chart extends MY_Model {
    ...
    function get_data($year, $table1, $table2, $field_group, $field_t1, $field_t2, $field_count, $data_name, $count_status, $status) {
        $query = $this->db->query(''
                . 'SELECT COUNT(' . $field_count . ') '
                . 'AS ' . $data_name . ' '
                . 'FROM ' . $table1 . ' , ' . $table2 . '  '
                . 'WHERE ' . $field_t2 . ' = ' . $field_t1 . ' '
                . 'AND YEAR(' . $field_group . ')=' . $year . ' '
                . 'AND ' . $count_status . ' = ' . $status . ' '
                . 'GROUP BY MONTH(' . $field_group . ')');
    
        return $query->result();
    }
    
    function get_monthly_data($year, $selling_point, $status, $or_status) {
        $query = $this->db->query('SELECT IFNULL(COUNT(stocks.id), 0) '
                . 'AS data '
                . 'FROM stocks, customers '
                . 'WHERE customers.id = stocks.customer_id '
                . 'AND YEAR(stocks.day_out)=' . $year . ' '
                . 'AND stocks.selling_point =' . $selling_point . ' '
                . 'AND (stocks.status =' . $status . ' '
                . 'OR stocks.status =' . $or_status . ') '
    ...
    public function get_year_sum($year){
        ...
        $query = $this->db->query('SELECT COUNT(stocks.id) AS data FROM stocks WHERE YEAR(day_in) = ' . $year); 
        
        return $query->result();        
    }

    function get_monthly_nosale($year, $selling_point, $status) {
        $query = $this->db->query('SELECT COUNT(customers.id) '
                . 'AS data FROM customers '
                . 'WHERE YEAR(customers.first_visit) =' . $year . ' '
                . 'AND customers.selling_point=' . $selling_point . ' '
                . 'AND customers.status =' . $status . ' '
                . 'GROUP BY MONTH(customers.first_visit)');
        return $query->result();
    }
    ...
    function get_doctor_data($year = null, $selling_point = null) {
        ...
        // Προσθέτουμε τις συνθήκες μόνο αν δεν είναι NULL
        if (!is_null($year)) {
    ...
    function get_vendor_data($year) {
        $query = $this->db->query('SELECT vendors.name '
                . 'AS name, COUNT(stocks.serial) AS data FROM stocks, vendors '
                . 'WHERE stocks.vendor = vendors.id '
                . 'AND YEAR(stocks.day_out)=' . $year . ' '
